...
<div style="
  ...
  margin-bottom: 12px;
">
  <div style="font-weight: 900; font-size: 18px; margin-bottom: 8px; color: {fg};">
    {cat}
  </div>
</div>
""",
    ...
    # KPI 圖格（沿用 common_ui 的 st.metric 風格）
    render_kpis(
        [
            KPI("有效貨位", f"{valid:,}"),
            KPI("已使用貨位", f"{used:,}"),
...
def main():
    ...
    inject_logistics_theme()
    set_page("儲位分類統計", icon="📦", subtitle="KPI + 圖表｜分類可調整｜使用率低於門檻整塊紅底")
