import streamlit as st
import pandas as pd
import datetime as dt
import re

from common_ui import (
    set_page,
    KPI,
    render_kpis,
    bar_topN,
    table_block,
    download_excel,
    card_open,
    card_close,
)

from qc_core import run_qc_efficiency


# ======================
# Time parser（關鍵）
# ======================
def _parse_time(text: str):
    """
    允許：
    - HH:MM
    - H:MM
    - HHMM
    回傳 datetime.time 或 None
    """
    if not text:
        return None

    text = text.strip()

    if re.fullmatch(r"\d{3,4}", text):
        text = text.zfill(4)
        text = f"{text[:2]}:{text[2:]}"

    try:
        return dt.datetime.strptime(text, "%H:%M").time()
    except ValueError:
        return None


# ======================
# Sidebar params
# ======================
def render_params():
    if "skip_rules" not in st.session_state:
        st.session_state.skip_rules = []

    st.caption("排除規則：時間必須自行輸入；未輸入代表全天不限制。")

    user = st.text_input("記錄輸入人（可空白＝全員）", value="")

    use_time = st.checkbox("啟用時間區間條件（自行輸入）", value=False)

    t_start = None
    t_end = None

    if use_time:
        c1, c2 = st.columns(2)
        with c1:
            t_start_txt = st.text_input(
                "開始時間（HH:MM）",
                placeholder="例如 10:30",
            )
        with c2:
            t_end_txt = st.text_input(
                "結束時間（HH:MM）",
                placeholder="例如 15:45",
            )

        t_start = _parse_time(t_start_txt)
        t_end = _parse_time(t_end_txt)

    c_add, c_clear = st.columns(2)

    with c_add:
        if st.button("➕ 加入排除規則"):
            if use_time:
                if t_start is None or t_end is None:
                    st.error("請自行輸入開始與結束時間（HH:MM）")
                else:
                    st.session_state.skip_rules.append(
                        {"user": user.strip(), "t_start": t_start, "t_end": t_end}
                    )
            else:
                st.session_state.skip_rules.append(
                    {"user": user.strip(), "t_start": None, "t_end": None}
                )

    with c_clear:
        if st.button("🧹 清空排除規則"):
            st.session_state.skip_rules = []

    if st.session_state.skip_rules:
        st.dataframe(
            pd.DataFrame(st.session_state.skip_rules),
            use_container_width=True,
            hide_index=True,
        )

    top_n = st.number_input("排行顯示人數", 10, 100, 30, step=10)

    return {
        "skip_rules": st.session_state.skip_rules,
        "top_n": int(top_n),
    }


# ======================
# helpers
# ======================
def _fmt(x, n=2):
    try:
        return f"{float(x):,.{n}f}"
    except Exception:
        return "—"


def _fmt_i(x):
    try:
        return f"{int(x):,}"
    except Exception:
        return "—"


def _build_kpis(df: pd.DataFrame, target: float):
    if df is None or df.empty:
        return dict(p=0, c=None, h=None, e=None, r=None)

    return dict(
        p=len(df),
        c=df["筆數"].sum() if "筆數" in df.columns else None,
        h=df["總工時"].sum() if "總工時" in df.columns else None,
        e=df["效率"].mean() if "效率" in df.columns else None,
        r=f"{(df['效率'] >= target).mean():.0%}" if "效率" in df.columns else None,
    )


def _seg(df, key):
    if df is None or df.empty:
        return df
    return df[df["時段"].astype(str).str.contains(key, na=False)].copy()


# ======================
# main
# ======================
def main():
    set_page("驗收達標效率", icon="✅")

    with st.sidebar:
        st.header("⚙️ 參數設定")
        params = render_params()

    # Upload
    card_open("📤 上傳資料")
    uploaded = st.file_uploader(
        "上傳驗收資料",
        type=["xlsx", "xls", "xlsm", "csv"],
        label_visibility="collapsed",
    )
    run = st.button("🚀 開始計算", type="primary", disabled=uploaded is None)
    card_close()

    if not run:
        st.info("請先上傳檔案")
        return

    with st.spinner("計算中..."):
        result = run_qc_efficiency(
            uploaded.getvalue(),
            uploaded.name,
            params["skip_rules"],
        )

    full_df = result.get("full_df", pd.DataFrame())
    ampm_df = result.get("ampm_df", pd.DataFrame())
    idle_df = result.get("idle_df", pd.DataFrame())

    target = float(result.get("target_eff", 20.0))
    top_n = params["top_n"]

    if "時段" not in ampm_df.columns:
        st.error("AM/PM 資料缺少『時段』欄位")
        return

    am = _seg(ampm_df, "上午")
    pm = _seg(ampm_df, "下午")

    am_idle = _seg(idle_df, "上午")
    pm_idle = _seg(idle_df, "下午")

    tab_am, tab_pm = st.tabs(["🌓 上午", "🌙 下午"])

    def render_seg(title, df, idle):
        k = _build_kpis(df, target)
        card_open(f"{title} KPI")
        render_kpis([
            KPI("人數", _fmt_i(k["p"]), "purple"),
            KPI("總筆數", _fmt_i(k["c"]), "blue"),
            KPI("總工時", _fmt(k["h"]), "cyan"),
            KPI("平均效率", _fmt(k["e"]), "teal"),
            KPI("達標率", k["r"] or "—", "gray"),
        ])
        card_close()

        if df.empty:
            st.info(f"{title} 無資料")
            return

        bar_topN(
            df,
            x_col="姓名",
            y_col="效率",
            hover_cols=["記錄輸入人", "筆數", "總工時"],
            top_n=top_n,
            target=target,
            title="",
        )

        table_block(
            summary_title=f"{title} 彙總",
            summary_df=df,
            detail_title=f"{title} 空窗明細",
            detail_df=idle,
        )

    with tab_am:
        render_seg("上午", am, am_idle)

    with tab_pm:
        render_seg("下午", pm, pm_idle)

    if result.get("xlsx_bytes"):
        card_open("⬇️ 匯出")
        download_excel(
            result["xlsx_bytes"],
            result.get("xlsx_name", "驗收達標_AMPM.xlsx"),
        )
        card_close()


if __name__ == "__main__":
    main()
